...
var frag = 'uniform sampler2D texture;\n\nvarying vec3 vColor;\n\nvoid main() {\n    gl_FragColor = vec4(vColor, 1.0);\n    // gl_FragColor = gl_FragColor * texture2D(texture, gl_PointCoord);\n    // if ( gl_FragColor.a < ALPHATEST ) discard;\n\n}\n';
...
var VolumeLayer = function (_Layer) {
    ...
    function VolumeLayer(props) {
        ...
        var _this = _possibleConstructorReturn(this, (VolumeLayer.__proto__ || Object.getPrototypeOf(VolumeLayer)).call(this, props));

        _this.data = props.data;
        _this.shape = props.shape;
        _this.mask = props.mask;
        _this.scale = props.scale || { x: 1, y: 1, z: 1 };
        _this.origin = props.origin || { x: 0, y: 0, z: 0 };
        _this.radius = 5;
        return _this;
    }
            ...
            for (var i = 0; i < self.shape[0]; i++) {
                for (var j = 0; j < self.shape[1]; j++) {
                    for (var k = 0; k < self.shape[2]; k++) {
                        ...
                        var dataIndex = (i * self.shape[1] + j) * self.shape[2] + k;
